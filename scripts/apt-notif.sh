...
PING_CHANNEL=$(cat "$DOCKERDIR/secrets/discord-channel-id" 2>/dev/null)
if [[ -z "$PING_CHANNEL" ]]; then echo "Discord Channel Required" 1>&2; exit 1; fi

APIKEY=$(cat "$DOCKERDIR/secrets/notifiarr-passthru-key" 2>/dev/null)
if [[ -z "$APIKEY" ]]; then echo "Notifiarr API Key Required" 1>&2; exit 1; fi

PING_USER=$(cat "$DOCKERDIR/secrets/discord-user-id" 2>/dev/null)
if [[ -z "$PING_USER" ]]; then PING_USER=0; fi

parallel-ssh \
	--timeout 0 \
	--hosts $DOCKERDIR/pssh/all.hosts \
...
envsubst < "$DOCKERDIR/scripts/apt-notif.json" >"$TMPFILE"
